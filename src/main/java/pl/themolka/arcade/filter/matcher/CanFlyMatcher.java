...
public class CanFlyMatcher extends ConfigurableMatcher<Boolean> {
    protected CanFlyMatcher(Config config) {
    ...
    public boolean matches(Player bukkit) {
        return bukkit != null && this.matches(bukkit.getAllowFlight());
    }
    ...
    public static class MatcherParser extends BaseMatcherParser<Config> implements InstallableParser {
        ...
        protected Result<Config> parseNode(Context context, Node node, String name, String value) throws ParserException {
            boolean canFly = this.canFlyParser.parseWithDefinition(context, node, name, value).orDefault(true);

            return Result.fine(node, name, value, new Config() {
                public Ref<Boolean> value() { return Ref.ofProvided(canFly); }
            });
        }
    }
