...
public class MaterialMatcher extends ConfigurableMatcher<Material> {
    ...
    public boolean matches(BlockState blockState) {
        return blockState != null && this.matches(blockState.getType());
    }

    public boolean matches(ItemStack itemStack) {
        return itemStack != null && this.matches(itemStack.getType());
    }
    ...
    public static class MatcherParser extends BaseMatcherParser<Config>
        ...
        protected Result<Config> parseNode(Context context, Node node, String name, String value) throws ParserException {
            Material material = this.materialParser.parseWithDefinition(context, node, name, value).orFail();

            return Result.fine(node, name, value, new Config() {
                public Ref<Material> value() { return Ref.ofProvided(material); }
            });
        }
    }

    public interface Config extends ConfigurableMatcher.Config<MaterialMatcher, Material> {
        @Override
