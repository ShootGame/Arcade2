...
public class CrouchingMatcher extends ConfigurableMatcher<Boolean> {
    protected CrouchingMatcher(Config config) {
    ...
    public boolean matches(Player bukkit) {
        return bukkit != null && this.matches(bukkit.isSneaking());
    }

    @NestedParserName({"crouching", "sneaking", "shifting"})
    @Produces(Config.class)
    public static class MatcherParser extends BaseMatcherParser<Config> implements InstallableParser {
        private Parser<Boolean> crouchingParser;

        @Override
        public void install(ParserLibrary library) throws ParserNotSupportedException {
            super.install(library);
            this.crouchingParser = library.type(Boolean.class);
        }

        @Override
        protected Result<Config> parseNode(Context context, Node node, String name, String value) throws ParserException {
            boolean crouching = this.crouchingParser.parseWithDefinition(context, node, name, value).orDefault(true);

            return Result.fine(node, name, value, new Config() {
                public Ref<Boolean> value() { return Ref.ofProvided(crouching); }
            });
        }
    }
